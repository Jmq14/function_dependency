...
def computeSingletonPartitions(listofcols):
	...
	for a in listofcols:
		dictpartitions[a]=[]
		for element in list_duplicates([row[ord(a)] for row in data2D]): # list_duplicates returns 2-tuples, where 1st is a value, and 2nd is a list of indices where that value occurs
...
def outputFD():
	...
	for fd in finallistofFDs:
		print "{} -> {}".format(' '.join(sorted(map(lambda x: str(ord(x)+1),fd[0]))), 
			str(ord(fd[1])+1))
...
if len(sys.argv) > 1:
	...
	print "Usage: python df.py [input text name]"
	exit()

data2D = [r.strip('\n').split(',') for r in open(infile, 'r').readlines()]
totaltuples = len(data2D)
if totaltuples <= 0:
	print "Empty file!"
	exit()
cols = len(data2D[0])
listofcolumns = map(lambda x: chr(x),list(range(cols)))
